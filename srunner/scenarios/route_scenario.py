...
class RouteScenario(BasicScenario):
    ...
    def _update_ego_vehicle(self):
        ...
        ego_vehicle = CarlaDataProvider.request_new_actor('vehicle.dodge.charger_2020',
                                                          elevate_transform,
                                                          rolename='hero')
    ...
    def _get_actors_instances(self, list_of_antagonist_actors):
        ...
        def get_actors_from_list(list_of_actor_def):
            ...
            for actor_def in list_of_actor_def:
                    ...
                    if actor.model != 'vehicle.carlamotors.firetruck':
                       sublist_of_actors.append(actor)
                    else:
                       print(f"[INFO] Skipping scenario firetruck actor at {actor.transform.location}")
    ...
    def _initialize_actors(self, config):
        # Final cleanup: remove any remaining firetrucks
        world = CarlaDataProvider.get_world()
        firetrucks = [v for v in world.get_actors().filter('vehicle.*') if v.type_id == 'vehicle.carlamotors.firetruck']

        for truck in firetrucks:
            print(f"[FINAL CLEANUP] Removing leftover firetruck: {truck.id}")
            truck.destroy()
        
        # Speed up traffic light cycles to avoid delays
        traffic_lights = world.get_actors().filter('traffic.traffic_light')
        for light in traffic_lights:
            try:
                light.set_green_time(3.5)
                light.set_red_time(2.0)
                light.set_yellow_time(2.0)
                # print(f"[TRAFFIC LIGHT] Adjusted timing at {light.get_transform().location}")
            except RuntimeError as e:
                print(f"[WARNING] Couldn't update traffic light: {e}")
        """
        ...
        tm_port = int(config.traffic_manager_port) if hasattr(config, 'traffic_manager_port') else 8000
        tm = CarlaDataProvider.get_client().get_trafficmanager(tm_port)
        tm.set_global_distance_to_leading_vehicle(2.5)  # Optional: set safer following distance
        tm.global_percentage_speed_difference(50.0)  # Vehicles move at 50% speed
        ...
        for _actor in new_actors:    
                if _actor.type_id == 'vehicle.carlamotors.firetruck':
                   print(f"[INFO] Removing firetruck: {_actor.id}")
                   _actor.destroy()
                else:
